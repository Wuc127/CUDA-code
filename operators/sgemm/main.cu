...
void random_init(float *data, int num_elements)
{
    ...
    {
        data[i] = 2.0f * static_cast<float>(rand()) / static_cast<float>(RAND_MAX) - 1.0f;
    }
}
...
int main()
{
    ...
    const size_t A_bytes = static_cast<size_t>(A_num_elements) * sizeof(float);
    const size_t B_bytes = static_cast<size_t>(B_num_elements) * sizeof(float);
    const size_t C_bytes = static_cast<size_t>(C_num_elements) * sizeof(float);

    // 申请主机内存
    float *A_host = static_cast<float *>(malloc(A_bytes));
    float *B_host = static_cast<float *>(malloc(B_bytes));
    float *C_cpu = static_cast<float *>(malloc(C_bytes));
    float *C_gpu_host = static_cast<float *>(malloc(C_bytes));
