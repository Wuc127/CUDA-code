...
int main()
{
    ...
    const size_t input_bytes = size * sizeof(float);
    const size_t output_bytes = blocks * sizeof(float);

    // 分配 CPU 内存
    float *input_host = static_cast<float *>(malloc(input_bytes));
    float *output_host = static_cast<float *>(malloc(output_bytes));
    ...
    // CPU 计算
    float cpu_result = reduce_cpu(input_host, size);

    // 分配 GPU 内存
    float *input_device = nullptr;
    float *output_device = nullptr;
    CHECK_CUDA(cudaMalloc(reinterpret_cast<void **>(&input_device), input_bytes));
    CHECK_CUDA(cudaMalloc(reinterpret_cast<void **>(&output_device), output_bytes));
    ...
    float gpu_result_v0 = sum_block_results(output_host, blocks);

    printf("Reduce v0:\n");

    reduce_check(&cpu_result, &gpu_result_v0);
    ...
    float gpu_result_v1 = sum_block_results(output_host, blocks);
    printf("Reduce v1:\n");
    reduce_check(&cpu_result,&gpu_result_v1);
