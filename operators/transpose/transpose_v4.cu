...
// 使用较少线程循环搬运 64 × 64 Tile，结合 float4 和 Padding 完成转置
...
)
{
    ...
    // block 为 16 × 8，每个线程分多轮搬运 Tile 中的不同区域
    for (int tile_row = ty; tile_row < TILE_DIM; tile_row += blockDim.y)
    {
